...
async function originalData(input) {
  const response = await fetch(input)   // Reading the file using default 
  const data = await response.text()    // Convert the data to String and 
  const array = data.split('\r')        // split it in an array 
